...
from rest_framework import viewsets, response, status, serializers, renderers
...
# from api.permissions import IsCandidate, IsEmployer
...
class EmployerViewset(DjoserUserViewSet):
    ...
    #     return response.Response(data, template_name="user/signup.html")

    # def your_method(self, request):
    #     if request.method == 'POST':
    #         form = YourForm(request.POST)
    #     if form.is_valid():
    #         # ваши логика и обработка формы
    #     else:
    #         form = YourForm()
...
class VacancyViewset(viewsets.ModelViewSet):
    ...
    serializer_class = VacancyReadListSerializer
    # permission_classes = (IsAuthor,)
    pagination_class = LimitPageNumberPagination
